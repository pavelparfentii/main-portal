...
class UpdateWeekSumTelegram extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:update-week-sum-telegram';
    ...
    public function handle()
    {
        ...
        $accountsWeekAgo = DB::connection('pgsql_telegrams')->table('accounts')
...
//            var_dump($account->two_weeks_ago_position);
            ...
            DB::connection('pgsql_telegrams')->table('accounts')
                ->where('id', $account->id)
//                ->where('week_number', $previousWeekNumber)
        ...
        $accounts = DB::connection('pgsql_telegrams')->table('accounts')
